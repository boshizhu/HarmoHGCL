...
def load_acm(ratio, type_num):
    ...
    feat_p = th.FloatTensor(feat_p.A)
    feat_a = th.FloatTensor(feat_a.A)
    feat_s = th.FloatTensor(feat_s.A)
    # feat_a = th.randn([type_num[1],128])
    # feat_s = th.randn([type_num[2],128])
...
def load_dblp(ratio, type_num):
    ...
    nei_c = np.load(path + "nei_c.npy", allow_pickle=True)
    nei_t = np.load(path + "nei_t.npy", allow_pickle=True)
    ...
    feat_p = th.FloatTensor(feat_p.A)
    feat_a = th.FloatTensor(feat_a.A)
    feat_c = th.FloatTensor(feat_c.A)
    feat_t = th.FloatTensor(feat_t.A)
    # feat_p = th.FloatTensor(preprocess_features(feat_p))
    # feat_a = th.FloatTensor(preprocess_features(feat_a))
    ...
    val = [th.LongTensor(i) for i in val]
    test = [th.LongTensor(i) for i in test]
    return [nei_p,nei_t,nei_c], [feat_a, feat_p,feat_t,feat_c], [apa, apcpa, aptpa], pos, label, train, val, test
