...
def main(args):
    ADJ, NS, MG, features, labels, num_classes, train_idx, val_idx, test_idx, type_mask = load_IMDB_data()
    ...
    for cur_repeat in range(args['repeat']):
        ...
        cprint("## Done ##", "yellow")
        model = MultiHGN(topo_dim, args['hidden_units'], in_dims, args['feat_drop'], args['att_drop'], len(MG),num_classes).to(args['device'])
        ...
        svm_macro, svm_micro, nmi, ari = evaluate_results_nc(h.detach().cpu().numpy(), labels.cpu().numpy(),
                                                             int(labels.max()) + 1)  # 使用SVM评估节点
        svm_macro_avg = svm_macro_avg + svm_macro
        svm_micro_avg = svm_micro_avg + svm_micro
        nmi_avg += nmi
        ari_avg += ari
        # with open('results_imdb.txt', 'a') as file:
...
if __name__ == '__main__':
    ...
    for i in range(1):
        for j in range(1):
            ...
            parser.add_argument('--margin2', type=float, default=0.7, help='')#0.3

            parser.add_argument('--w_loss1', type=float, default=0, help='')#0
            parser.add_argument('--w_loss2', type=float, default=1, help='')#1000最好
