...
def main(args):
    ...
    ADJ, NS, MG, features, labels, num_classes, train_idx, val_idx, test_idx, type_mask,pos = load_DBLP_data()
    ...
    for cur_repeat in range(args['repeat']):
        ...
        num_neg = 4  # 4
        lbl_z = torch.tensor([0.]).to('cuda:0')
        ...
        nmi_avg += nmi
        ari_avg += ari
        with open('results_dblp.txt', 'a') as file:
        ...
        node_pos = ml.fit_transform(h.detach().cpu().numpy())
        color_idx = {}
        for i in range(len(h.detach().cpu().numpy())):
        ...
        plt.legend()
        plt.savefig(".\\visualization\HarmoHGCL" + str(args['dataset']) + "分类图" + str(cur_repeat) + ".png", dpi=1000,
        			bbox_inches='tight')
        plt.show()
    ...
    print('mic=', micro_f1, '  mac=', macro_f1, 'acc=', accuracy)
...
if __name__ == '__main__':
    ...
    for i in range(1):
        for j in range(1):
            ...
            parser.add_argument('--dataset', default='DBLP', help='数据集')
            parser.add_argument('--lr', default=0.001, help='学习率') #imdb=0.002 #ACM=0.002 #DBLP=0.001
            ...
            parser.add_argument('--margin1', type=float, default=1, help='')#0.8
            parser.add_argument('--margin2', type=float, default=1, help='')#0.9
            ...
            parser.add_argument('--w_loss1', type=float, default=20, help='')#20
            parser.add_argument('--w_loss2', type=float, default=20, help='')#20
